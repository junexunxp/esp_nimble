...
#if TEST_USE_WVT
			sprintf((char*)log_msg,"loop : %d, events endcrypt %ld, events error %ld, events micstatus %ld\n",loop, NRF_CCM->EVENTS_ENDCRYPT,NRF_CCM->EVENTS_ERROR,NRF_CCM->MICSTATUS);
			SET_DBG_DATA((uint8_t *)&log_msg, strlen((const char*)log_msg));
			SET_DBG_MSG("ccm rx test failed\n");
			TEST_SIGNAL(SIGNAL_ID_DEFAULT, DATA_FMT_STRING);
#endif
...
TEST_CASE(hal_radio_tc_01)
{
	...
	SET_DBG_DATA((uint8_t *)&log_msg, strlen((const char*)log_msg));
	SET_DBG_MSG("test ccm RX decrypt function\n");
	TEST_SIGNAL(SIGNAL_ID_DEFAULT, DATA_FMT_STRING);
	hal_radio_ccm_rx_test();
}

TEST_CASE(hal_radio_tc_02)
{
	sprintf((char*)log_msg,"\n %s started\n",__func__);
	SET_DBG_DATA((uint8_t *)&log_msg, strlen((const char*)log_msg));
	SET_DBG_MSG("test ccm TX encrypt function\n");
	TEST_SIGNAL(SIGNAL_ID_DEFAULT, DATA_FMT_STRING);
	hal_radio_ccm_tx_test();
}


TEST_CASE(hal_radio_tc_04)
{
	sprintf((char*)log_msg,"\n %s started\n",__func__);
	SET_DBG_DATA((uint8_t *)&log_msg, strlen((const char*)log_msg));
	SET_DBG_MSG("test ccm en/decrypt function\n");
	TEST_SIGNAL(SIGNAL_ID_DEFAULT, DATA_FMT_STRING);
	hal_ccm_endecrypt_test();
}

TEST_CASE(hal_radio_tc_03)
{
	sprintf((char*)log_msg,"\n %s started\n",__func__);
	SET_DBG_DATA((uint8_t *)&log_msg, strlen((const char*)log_msg));
