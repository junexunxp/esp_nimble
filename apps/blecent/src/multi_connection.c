...
#define NOTIFY_LEN			490//490
//static volatile uint8_t app_buffer[1024];
static int blecent_gap_event(struct ble_gap_event *event, void *arg);
#if 0

static void
blecent_throughput_test(uint16_t connhandle, uint16_t atthandle);
...
blecent_on_write(uint16_t conn_handle,
                 ...
                 void *arg)
{
    MODLOG_DFLT(INFO, "Write complete; status=%d conn_handle=%d "
                      "attr_handle=%d\n",
...
blecent_read_write_subscribe(const struct peer *peer)
{
	...
	blecent_throughput_test(peer->conn_handle, chr->chr.val_handle);
...
//For throughput test
uint8_t throughputV[251];
const struct peer *ppeer = NULL;
const struct peer_chr *pchr = NULL;



static int
blecent_att_write_done(uint16_t conn_handle,
                 const struct ble_gatt_error *error,
                 struct ble_gatt_attr *attr,
                 void *arg)
{
    MODLOG_DFLT(INFO, "Write complete "
                      "\n");
	blecent_throughput_test(conn_handle,attr->handle);
...
blecent_throughput_test(uint16_t connhandle, uint16_t atthandle)
{
    ...
    ble_gattc_write_flat(connhandle, atthandle,
...
static void
blecent_throughput_client_start(const struct peer *peer)
{
    const struct peer_dsc *dsc;
    uint8_t value[2];
...
void bletest_show_rslt(void ){
	...
	uint32_t txed_bytes = packets_cnt*NOTIFY_LEN;
...
blecent_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC:
		...
		//Modify @June
		//Continue scan for more connection
		blecent_scan();
        return 0;
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        MODLOG_DFLT(INFO, "\n");
	//	if(app_buffer[1023]==1){
	//		app_buffer[1000] = 0xfe;
	//	}
        /* Forget about peer. */
    ...
    case BLE_GAP_EVENT_NOTIFY_RX:
		...
		if((OS_MBUF_PKTLEN(event->notify_rx.om) == NOTIFY_LEN)/* && (event->notify_rx.om->om_data[NOTIFY_LEN - 1]) == check_sum(event->notify_rx.om->om_data,NOTIFY_LEN - 1)*/){
			rxed_pkt();
		}
        return 0;
...
#if 0
	uint8_t ij;
	for(ij=0;ij<sizeof throughputV;ij++){
